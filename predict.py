...
dataprueba = data_modeling.iloc[118:120,:]
encoder_load = pickle.load(open('models/encode_final', "rb"))
data_modeling3=encoder_load.transform(dataprueba)
X = data_modeling3.drop(columns=['fraudfound_p', 'policynumber'])
Model = pickle.load(open('models/Model_Fraude.pkl', 'rb'))
result = Model.predict_proba(X)[:, 1]
Fraude_Final_predict = pd.concat([dataprueba['policynumber'].reset_index()['policynumber'],pd.DataFrame({'Predict' : result})],axis=1)
print(Fraude_Final_predict)